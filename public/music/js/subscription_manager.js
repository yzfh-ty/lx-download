  ...
  const SubscriptionManager = {
    ...
    async load(force = false) {
      ...
      try {
        ...
        this.settings.navidromeApiMode = data.settings?.navidromeApiMode === true
        this.loaded = true
    ...
    async renderSettingsPanel() {
          ...
          <div class="flex flex-wrap items-center justify-between gap-3 py-2 border-b t-border-main" data-sub-id="${sub.id}">
            <div class="min-w-0 flex-1">
              ...
              <div class="text-xs mt-1 ${sub.playlistLastError ? 'text-red-500' : 't-text-muted'}">${sub.playlistLastError
                ...
                : sub.playlistUpdatedAt ? `本地清单更新于 ${this._formatTime(sub.playlistUpdatedAt)}，${this.settings.navidromeApiMode ? '通过 Navidrome API 同步' : '等待 Navidrome 扫描'}${sub.enabled ? '' : '；订阅已暂停'}` : '等待生成本地清单'}</div>
      ...
      container.innerHTML = `
        <div class="t-bg-panel rounded-2xl shadow-sm border t-border-main p-4 sm:p-8 w-full min-h-full space-y-4">
          <p class="text-xs t-text-muted mb-4">订阅歌单或排行榜后，服务端自动下载歌曲并维护同名目录和清单，无需保持浏览器开启。Navidrome API 同步请在侧栏“Navidrome 设置”中配置；仅文件导入模式需要启用 M3U8 自动导入。远端移除歌曲或取消订阅时，已下载文件仍会保留。</p>
    ...
    async rebuild(id, button) {
      ...
      try {
        ...
        if (window.showSuccess) showSuccess(`本地清单已生成，包含 ${data.data.playlistTrackCount || 0} 首歌曲；${this.settings.navidromeApiMode ? 'API 同步状态见 Navidrome 设置页' : 'Navidrome 扫描后生效'}`)
      } catch (e) {
