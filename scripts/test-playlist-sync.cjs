...
(async () => {
  await check('API mode suppresses generated M3U8 imports, including paused playlists, and preserves rename targets', async () => {
    const h = harness('api-exports'); h.add('A'); const sub = await h.subscribe();
    const oldFile = path.join(h.root, sub.playlistPath); const oldUnmatched = path.join(h.root, h.api.getUnmatchedPlaylist().playlistPath);
    await h.api.update('shared', sub.id, { enabled: false }); h.deps.apiSyncMode = () => true; h.api.reconcilePlaylists();
    assert(!fs.existsSync(oldFile)); assert(!fs.existsSync(oldUnmatched)); assert(fs.existsSync(oldFile + '.lx-export'));
    assert.equal(h.api.getNavidromeSyncPlans().find(item => item.id === sub.id).enabled, false);
    await h.api.update('shared', sub.id, { enabled: true });
    const plans = h.api.getNavidromeSyncPlans(); assert.equal(plans.find(item => item.id === sub.id).paths.length, 1);
    const renamed = h.api.renameNavidromePlaylist(sub.id, 'API改名');
    assert.equal(renamed.playlistPath, 'API改名/API改名.m3u8.lx-export'); assert(!fs.existsSync(path.join(h.root, 'API改名/API改名.m3u8')));
    assert(fs.existsSync(path.join(h.root, 'API改名/A.flac'))); h.api.stop();
    const restored = harness('unused', h.database, h.root); assert.equal(restored.view(sub.id).playlistPath, renamed.playlistPath);
  });
  await check('API export migration refuses foreign file contents without touching music', async () => {
    const h = harness('api-foreign-export'); h.add('A'); const sub = await h.subscribe();
    fs.writeFileSync(path.join(h.root, sub.playlistPath), 'USER CONTENT');
    h.deps.apiSyncMode = () => true; h.api.reconcilePlaylists();
    assert(h.api.getNavidromeSyncPlans().find(item => item.id === sub.id).error);
    assert.equal(fs.readFileSync(path.join(h.root, sub.playlistPath), 'utf8'), 'USER CONTENT');
