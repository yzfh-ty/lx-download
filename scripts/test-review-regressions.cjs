...
function fileCacheHarness(folder, taggerClass) {
  ...
  const fakeRequire = name => {
    ...
    if (name === './embeddedCover') return evaluate(fs.readFileSync(path.join(root, 'src/server/embeddedCover.ts'), 'utf8'), [], {
      require: id => id === 'music-metadata' ? { parseFile: (...args) => import('music-metadata').then(api => api.parseFile(...args)) } : require(id)
    }).exports;
    if (name === 'music-tag-native') return { MusicTagger: taggerClass || class { loadPath() { throw new Error('Native tags excluded from regression fixtures'); } dispose() {} }, MetaPicture: class {} };
...
async function main() {
  await check('MP3 and FLAC scans and cover serving avoid native picture access', async () => {
    let nativePictureReads = 0;
    class MetadataTagger {
      loadPath() { this.duration = 60; this.bitRate = 128; this.sampleRate = 44100; this.bitDepth = 16; this.lyrics = ''; }
      get pictures() { nativePictureReads++; throw new Error('Native cover getter must not be used'); }
      dispose() {}
    }
    const { api } = fileCacheHarness('cover-reader', MetadataTagger);
    ...
    const audioFrames = Buffer.concat(Array.from({ length: 5 }, () => frame));
    const id3 = require('node-id3').create({ title: 'Cover fixture', image: { mime: 'image/png', type: { id: 3 }, description: '', imageBuffer: png } });
    fs.writeFileSync(path.join(music, 'cover.mp3'), Buffer.concat([id3, audioFrames]));
    fs.writeFileSync(path.join(music, 'no-cover.mp3'), audioFrames);
    fs.writeFileSync(path.join(music, 'broken.mp3'), 'damaged file');
    const uint32 = value => { const buffer = Buffer.alloc(4); buffer.writeUInt32BE(value); return buffer; };
    const info = Buffer.alloc(34); info.writeUInt16BE(4096, 0); info.writeUInt16BE(4096, 2);
    info.writeBigUInt64BE((44100n << 44n) | (1n << 41n) | (15n << 36n) | 44100n, 10);
    const picture = Buffer.concat([uint32(3), uint32(9), Buffer.from('image/png'), uint32(0), uint32(1), uint32(1), uint32(32), uint32(0), uint32(png.length), png]);
    const pictureHeader = Buffer.alloc(4); pictureHeader[0] = 0x86; pictureHeader.writeUIntBE(picture.length, 1, 3);
    fs.writeFileSync(path.join(music, 'cover.flac'), Buffer.concat([Buffer.from('fLaC'), Buffer.from([0, 0, 0, 34]), info, pictureHeader, picture]));
    await api.syncCacheIndex('shared');
    for (const filename of ['cover.mp3', 'cover.flac']) {
      const item = api.indexManager.getAll('shared', 'music').find(item => item.filename === filename);
      assert.equal(item.hasCover, true); assert.equal(item.coverType, 'embedded');
      for (let read = 0; read < 2; read++) {
        const cover = await api.getCacheCover(filename, 'shared');
        assert.equal(cover.mime, 'image/png'); assert(cover.data.equals(png));
      }
      item.coverCheckedVersion = undefined; api.indexManager.save('shared', 'music');
    }
    await api.syncCacheIndex('shared');
    for (const filename of ['no-cover.mp3', 'broken.mp3']) assert.equal(await api.getCacheCover(filename, 'shared'), null);
    assert.equal(nativePictureReads, 0);
  });
  for (const failure of ['load', 'lyrics']) {
    await check(`Metadata scan disposes native resources after ${failure} failure`, async () => {
      let allocated = 0, disposed = 0;
      class FailingTagger {
        constructor() { allocated++; }
        loadPath() { if (failure === 'load') throw new Error('fixture load failure'); }
        get lyrics() { if (failure === 'lyrics') throw new Error('fixture lyric failure'); return ''; }
        dispose() { if (!this.closed) { this.closed = true; disposed++; } }
      }
      const { api } = fileCacheHarness('metadata-cleanup-' + failure, FailingTagger);
      fs.writeFileSync(path.join(api.getCacheDir('shared', true), 'broken.flac'), audio(1));
      for (let scan = 0; scan < 5; scan++) {
        await api.syncCacheIndex('shared');
        assert(allocated > 0);
        assert.equal(disposed, allocated, 'every allocated tagger must be released even when reading throws');
      }
    });
  }
  ...
  await check('R06/R07/R11 real HTTP authentication and file boundary regression', async () => {
    ...
    const module = evaluate(source, [], {
      ...
      require: name => {
        ...
        if (name === './navidromeSync') return {
          getSettings: () => ({ url: 'http://fixture.invalid', username: 'fixture-user', hasPassword: true }), getStatuses: () => ({}),
          saveSettings: () => ({ enabled: true }), testConnection: async () => ({ message: 'OK' }), syncNow: async () => ({ statuses: {} }), bindPlaylist: async () => ({ playlistId: 'fixture' }),
          listRemotePlaylists: async () => ({ scope: 'fixture-scope', playlists: [] }),
          deleteRemotePlaylists: async input => { navidromeDeletes.push(input); return { deleted: input.playlistIds, missing: [], failed: [] }; }
        };
    ...
    try {
      ...
      assert.equal((await fetch(origin + '/api/music/navidrome/settings', { headers: { Origin: origin } })).status, 401);
      const navConfig = await fetch(origin + '/api/music/navidrome/settings', { headers }); assert.equal(navConfig.status, 200); assert.equal((await navConfig.json()).data.hasPassword, true);
      for (const action of ['settings', 'test', 'sync', 'bind']) {
        const url = origin + '/api/music/navidrome/' + action;
        assert.equal((await fetch(url, { method: 'POST', headers: { Origin: origin }, body: '{}' })).status, 401);
        assert.equal((await fetch(url, { method: 'POST', headers: { ...headers, Origin: 'https://cross-site.invalid' }, body: '{}' })).status, 403);
        assert.equal((await fetch(url, { method: 'POST', headers, body: '{}' })).status, 200);
      }
      const remoteListUrl = origin + '/api/music/navidrome/remote-playlists';
      assert.equal((await fetch(remoteListUrl, { headers: { Origin: origin } })).status, 401);
      const remoteList = await fetch(remoteListUrl, { headers });
      assert.equal(remoteList.status, 200); assert.equal(remoteList.headers.get('cache-control'), 'no-store');
      assert.equal((await remoteList.json()).data.scope, 'fixture-scope');
      const deletion = { scope: 'fixture-scope', playlistIds: ['legacy-one'] };
      const deleteUrl = remoteListUrl + '/delete';
      assert.equal((await fetch(deleteUrl, { method: 'POST', headers: { Origin: origin }, body: JSON.stringify(deletion) })).status, 401);
      assert.equal((await fetch(deleteUrl, { method: 'POST', headers: { ...headers, Origin: 'https://cross-site.invalid' }, body: JSON.stringify(deletion) })).status, 403);
      assert.equal(navidromeDeletes.length, 0);
      assert.equal((await fetch(deleteUrl, { method: 'POST', headers, body: JSON.stringify(deletion) })).status, 200);
      assert.equal(JSON.stringify(navidromeDeletes), JSON.stringify([deletion]));
