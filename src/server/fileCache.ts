...
import { detectImageMime, readEmbeddedCover } from './embeddedCover'
...
const readEmbeddedCoverState = async (filePath: string) => !!(await readEmbeddedCover(filePath))
        ...
        for (const file of files) {
            ...
            if (!existing || existing.size !== stats.size || existing.hasLyric !== hasLyricOnDisk || needsCoverCheck || !existing.interval || existing.quality === 'unknown' || !existing.bitrate || qualityCorrectionNeeded) {
                if (existing) {
                    ...
                    if (needsCoverCheck) {
                        const hasEmbeddedCover = await readEmbeddedCoverState(filePath)
                    ...
                    hasCover = await readEmbeddedCoverState(filePath)
    ...
    for (const loc of locations) {
        for (const folder of roots) {
            ...
            if (filePath && fs.existsSync(filePath)) {
                ...
                const embeddedCover = await readEmbeddedCover(filePath)
                if (embeddedCover) {
                    writeCoverCache(filename, normalizedUsername, embeddedCover.data, embeddedCover.mime, stats)
    ...
    if (result.exists && !result.isCollision) {
        ...
        if (isOnlyDownload && result.folder === 'cache' && result.path) {
            ...
            try {
                ...
                // The copied audio remains usable without readable metadata.
            } finally {
                try { if (tagger) tagger.dispose() } catch (e) { }
            }
            hasCover = await readEmbeddedCoverState(finalPath)
        ...
        const downloadFrom = (currentUrl: string) => {
            ...
            fileStream.on('close', async () => {
                ...
                installDownloadedFile(tempPath, dir, preferredName, ext).then(async ({ finalPath }) => {
                    ...
                    const hasEmbeddedCover = await readEmbeddedCoverState(finalPath)
                    let finalHasCover = hasEmbeddedCover
                    ...
                    if (taggedItem) {
                        taggedItem.coverType = hasEmbeddedCover
                            ? 'embedded'
                            : finalHasCover
                                ? 'cached'
...
export const replaceDownloadedMusicItem = async (
    ...
    try {
        ...
        const stagedHasCover = await readEmbeddedCoverState(sourceAudioPath)
        ...
        let finalHasCover = await readEmbeddedCoverState(targetAudioPath)
        if (!finalHasCover && originalCover?.data?.length) {
            ...
            finalHasCover = await readEmbeddedCoverState(targetAudioPath)
        }
        ...
        replacementItem = {
            ...
            coverType: (await readEmbeddedCoverState(targetAudioPath)) ? 'embedded' : finalHasCover ? 'cached' : hasUsableRemoteCover(downloadedItem.img) ? 'remote' : 'none',
            coverCheckedVersion: COVER_CHECK_VERSION,
