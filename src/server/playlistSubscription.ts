...
interface SubscriptionState {
  ...
  unmatchedPlaylist?: playlistFiles.PlaylistFiles & { paths?: string[], nameCustomized?: boolean, playlistUpdatedAt?: number, playlistLastError?: string, playlistTrackCount?: number }
}
...
const ensureSubscriptionDirectory = (root: string, sub: PlaylistSubscription) => {
  ...
  if (sub.directoryName && deps?.apiSyncMode?.()) playlistFiles.disablePlaylistAutoImport(root, sub)
  ...
  if (deps?.apiSyncMode?.()) playlistFiles.disablePlaylistAutoImport(root, sub)
...
const syncUnmatchedPlaylist = (cachedSongs = deps?.getReadySongs?.() || []) => {
  ...
  try {
    ...
    if (files.directoryName && deps?.apiSyncMode?.()) playlistFiles.disablePlaylistAutoImport(root, files)
    ...
    if (deps?.apiSyncMode?.()) playlistFiles.disablePlaylistAutoImport(root, files)
...
export const reconcilePlaylists = () => {
  ...
  for (const sub of state.subscriptions) {
    ...
    else if (deps?.apiSyncMode?.() && deps.getDownloadRoot && sub.directoryName) {
      try { playlistFiles.disablePlaylistAutoImport(deps.getDownloadRoot(), sub) } catch (err: any) { sub.playlistLastError = err?.message || '清单切换失败' }
    }
  }
  syncUnmatchedPlaylist(cached)
}

export const getNavidromeSyncPlans = () => {
  if (!initialScanReady || !deps?.getDownloadRoot) return []
  const root = deps.getDownloadRoot()
  const safePaths = (paths: string[]) => paths.filter(path => playlistFiles.audioExists(root, path))
  const exportError = (filename?: string) => deps?.apiSyncMode?.() && !filename?.endsWith('.lx-export') ? '请先完成本地清单切换，避免 M3U8 重复导入' : ''
  const unmatched = state.unmatchedPlaylist
  return [
    ...state.subscriptions.map(sub => ({
      id: sub.id, name: sub.playlistName || sub.directoryName || sub.name,
      enabled: sub.enabled && !!sub.remoteTracks,
      paths: safePaths((sub.remoteTracks || []).map(track => track.localRelativePath || '').filter(Boolean)),
      error: sub.playlistLastError || exportError(sub.playlistFilename) || (sub.playlistRoot && sub.playlistRoot !== root ? '等待本地目录对账' : ''),
    })),
    ...(unmatched ? [{ id: UNMATCHED_ID, name: unmatched.name, enabled: true, paths: safePaths(unmatched.paths || []), error: unmatched.playlistLastError || exportError(unmatched.playlistFilename) }] : []),
  ]
}
...
export const getSettings = () => ({ intervalMinutes: state.intervalMinutes, navidromeApiMode: deps?.apiSyncMode?.() === true })
