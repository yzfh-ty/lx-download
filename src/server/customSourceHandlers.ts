...
// 站点会话鉴权器由 server.ts 注入(避免循环依赖),登录用户即为管理员
let authChecker: (req: IncomingMessage) => boolean = () => false
export const setAuthChecker = (fn: (req: IncomingMessage) => boolean) => { authChecker = fn }
...
export async function handleValidate(req: IncomingMessage, res: ServerResponse) {
    try {
        ...
        } as any, { register: false })
        // Validation only needs the metadata; release the probe's VM resources.
...
export async function handleImport(req: IncomingMessage, res: ServerResponse) {
    try {
        ...
        try {
            ...
            if (!['http:', 'https:'].includes(parsedUrl.protocol)) throw new Error('只支持 HTTP(S) 地址')
        } catch (err: any) {
            res.writeHead(400, { 'Content-Type': 'application/json' })
            res.end(JSON.stringify({ success: false, error: err.message || 'Invalid HTTP(S) URL' }))
            return
        }
        ...
        const download = async (targetUrl: string, depth = 0): Promise<string> => {
            ...
            const protocol = target.protocol === 'https:' ? require('https') : require('http')

            return new Promise((resolve, reject) => {
                const request = protocol.get(target, (response: any) => {
                    const { statusCode } = response
                    response.setTimeout(15000, () => response.destroy(new Error('远程脚本请求超时')))
                ...
                request.setTimeout(15000, () => request.destroy(new Error('远程脚本请求超时')))
...
export async function handleList(req: IncomingMessage, res: ServerResponse, username: string) {
    ...
    // Shared source list; keep the old variable names to minimize API changes.
    ...
    const enrichedSources = allSources.map((source: any) => {
        // 合并运行时状态
        // Shared Web sources are exposed as public sources in the list API,
        // while the runtime loader records their status under the shared owner.
        const status = getApiStatus(source.owner, source.id) || getApiStatus('shared', source.id)
        if (status) {
