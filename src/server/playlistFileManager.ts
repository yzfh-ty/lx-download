...
export const ensurePlaylistDirectory = (root: string, files: PlaylistFiles, reserved: string[] = []) => {
  ...
  if (path.basename(files.playlistFilename) !== files.playlistFilename || /[\\/]/.test(files.playlistFilename) || !/\.m3u8(?:\.lx-export)?$/.test(files.playlistFilename)) throw new Error('无效的歌单清单文件名')
  return dir
}

/** API-managed exports must not be rediscovered as new playlists by Navidrome's scanner. */
export const disablePlaylistAutoImport = (root: string, files: PlaylistFiles) => {
  ensurePlaylistDirectory(root, files)
  if (files.playlistFilename!.endsWith('.lx-export')) return
  const oldPath = safePath(root, `${files.directoryName}/${files.playlistFilename}`)
  const nextFilename = files.playlistFilename + '.lx-export'
  const nextPath = safePath(root, `${files.directoryName}/${nextFilename}`)
  if (fs.existsSync(nextPath)) {
    // A crash after rename but before saving state can leave only the expected export.
    if (fs.existsSync(oldPath) || !fs.readFileSync(nextPath, 'utf8').startsWith('#EXTM3U\n# 由 lx-download 自动生成，请勿手工编辑\n')) throw new Error('导出清单路径已被其他文件占用')
  } else if (fs.existsSync(oldPath)) {
    if (!fs.readFileSync(oldPath, 'utf8').startsWith('#EXTM3U\n# 由 lx-download 自动生成，请勿手工编辑\n')) throw new Error('歌单文件不是程序生成的清单，无法自动切换 API 模式')
...
export const renamePlaylistDirectory = (root: string, files: PlaylistFiles, name: string, reserved: string[]) => {
  ...
  const nextFilename = directory + (files.playlistFilename?.endsWith('.lx-export') ? '.m3u8.lx-export' : '.m3u8')
