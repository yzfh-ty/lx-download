...
function createLxRequest(runtime: SourceRuntime, isUnsafe: boolean = false) {
    return (url: string, options: any, callback: Function) => {
        ...
        let request: ReturnType<typeof needle.request> | undefined
        let completed = false
        const cancel = () => {
            runtime.finishRequest(cancel)
            const reqObj = (request as any)?.request
            if (reqObj && !reqObj.aborted) reqObj.abort()
        }
        request = needle.request(method, url, data, requestOptions, (err: any, resp: any, body: any) => {
            completed = true
            runtime.finishRequest(cancel)
            if (runtime.disposed) return
            try {
        ...
        if (!completed) runtime.trackRequest(cancel)
        return cancel
    }
}

// 加载自定义源脚本
export async function loadUserApi(apiInfo: UserApiInfo, options: { register?: boolean } = {}): Promise<any> {
    // 从脚本中提取元数据
    ...
    const apiKey = `${fullApiInfo.owner}_${apiInfo.id}`
    ...
    // A script can reject initialization before its asynchronous run finishes.
    ...
    const lxObject = {
        ...lxDataInside,
        utils: lxUtils,
        request: createLxRequest(runtime, !!apiInfo.allowUnsafeVM && !!global.lx.config['system.allowUnsafeVM']),
    ...
    const sandbox: any = {
        ...
        clearTimeout: runtime.clearTimer,
        setInterval: runtime.setInterval,
        clearInterval: runtime.clearTimer,
        Buffer,
        URL,
        URLSearchParams,
        TextEncoder,
        TextDecoder,
        process: {
            nextTick: (fn: Function, ...args: any[]) => runtime.setTimeout(() => fn(...args), 0),
    ...
    try {
        const initialize = async () => {
            if (apiInfo.allowUnsafeVM && global.lx.config['system.allowUnsafeVM']) {
                ...
                await vm.runInContext(apiInfo.script, context, {
        ...
        // Bound the entire initialization, including an asynchronous script run.
        await Promise.race([
            initialize(),
            new Promise((_, reject) => {
                initTimer = runtime.setTimeout(() => reject(new Error('初始化超时，请确保脚本调用了 lx.send("inited", ...)')), 3000)
            })
        ])
        ...
        const apiInstance = {
            ...
            dispose: () => {
                runtime.dispose()
                eventHandlers.clear()
                if (loadedApis.get(apiKey) === apiInstance) loadedApis.delete(apiKey)
            },
            callRequest: async (action: string, source: string, info: any) => {
                try {
                    if (runtime.disposed) throw new Error('自定义源已卸载')
                    ...
                    const result = await runtime.waitFor(handler(inputData))
        ...
        if (options.register !== false) {
            loadedApis.get(apiKey)?.dispose()
            loadedApis.set(apiKey, apiInstance)
        }
...
export async function callUserApiGetMusicUrl(
    ...
    // All enabled custom sources belong to the single shared Web scope.
    let candidates: any[] = Array.from(loadedApis.values()).filter(api => (
        api.info.enabled && api.info.sources && api.info.sources[source]
    ))
    ...
    if (candidates.length > 1) {
        ...
        const orderCandidates = [path.join(dataPath, 'source', 'order.json')]
...
function startWatcher(sourceRoot: string) {
    ...
    try {
        ...
        fsWatcher = fs.watch(sourceRoot, { recursive: true }, (eventType, filename) => {
            ...
            // The current single-Web layout stores sources directly under
            // sourceRoot, so root-level changes belong to the shared owner.
            let username = parts.length > 1 ? parts[0] : 'shared'
...
// Serialize reloads so overlapping file events cannot leave superseded VMs alive.
export function initUserApis(targetUser = '') {
    const reload = reloadQueue.then(() => reloadUserApis(targetUser))
    reloadQueue = reload.catch(() => {})
    return reload
}

// 从共享目录加载所有已启用的自定义源
async function reloadUserApis(targetUser: string) {
    const dataPath = process.env.DATA_PATH || path.join(process.cwd(), 'data')
    const sourceRoot = path.join(dataPath, 'source')
    const stats = { loadedCount: 0 }

    lastReloadMap.set(targetUser || 'shared', Date.now())
    lastReloadMap.set('shared', Date.now())

    for (const api of loadedApis.values()) api.dispose()
    loadedApis.clear()
    apiStatus.clear()
    ...
    // Single Web client: load one shared source manifest, regardless of the
    // legacy targetUser argument used by older callers.
    await loadSourcesFromDir(sourceRoot, 'shared', stats)
    lastReloadMap.set('shared', Date.now())
    console.log(`[UserApi] 本次加载: ${stats.loadedCount} 个共享源`)
...
export function isSourceSupported(source: string, clientUsername?: string): boolean {
    for (const [apiId, api] of loadedApis) {
        ...
        // 权限检查
        if (api.info.owner === 'open' || api.info.owner === 'shared') {
            return true
        }
    }
    return false
