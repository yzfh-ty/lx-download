...
import * as navidromeSync from './navidromeSync'
...
const handleStartServer = async (port = 9527, ip = '127.0.0.1') => await new Promise((resolve, reject) => {
  const httpServer = http.createServer(async (req, res) => {
    ...
    if (pathname.startsWith('/api/')) {
      ...
      if (pathname === '/api/music/navidrome/settings' && req.method === 'GET') {
        if (!getCacheRequestUsername(req)) {
          res.writeHead(401, { 'Content-Type': 'application/json' }); res.end(JSON.stringify({ success: false, message: 'Unauthorized' })); return
        }
        res.writeHead(200, { 'Content-Type': 'application/json', 'Cache-Control': 'no-store' })
        res.end(JSON.stringify({ success: true, data: navidromeSync.getSettings(), statuses: navidromeSync.getStatuses() }))
        return
      }
      if (pathname === '/api/music/navidrome/remote-playlists' && req.method === 'GET') {
        if (!getCacheRequestUsername(req)) {
          res.writeHead(401, { 'Content-Type': 'application/json' }); res.end(JSON.stringify({ success: false, message: 'Unauthorized' })); return
        }
        try {
          const data = await navidromeSync.listRemotePlaylists()
          res.writeHead(200, { 'Content-Type': 'application/json', 'Cache-Control': 'no-store' })
          res.end(JSON.stringify({ success: true, data }))
        } catch (err: any) {
          res.writeHead(400, { 'Content-Type': 'application/json' })
          res.end(JSON.stringify({ success: false, message: err?.message || '读取远端歌单失败' }))
        }
        return
      }
      if (['/api/music/navidrome/settings', '/api/music/navidrome/test', '/api/music/navidrome/sync', '/api/music/navidrome/bind', '/api/music/navidrome/remote-playlists/delete'].includes(pathname) && req.method === 'POST') {
        if (!getCacheRequestUsername(req)) {
          res.writeHead(401, { 'Content-Type': 'application/json' }); res.end(JSON.stringify({ success: false, message: 'Unauthorized' })); return
        }
        await readBody(req).then(async body => {
          try {
            ...
            const data = pathname.endsWith('/settings') ? navidromeSync.saveSettings(input)
              : pathname.endsWith('/test') ? await navidromeSync.testConnection(input)
                : pathname.endsWith('/bind') ? await navidromeSync.bindPlaylist(String(input.id || ''), String(input.playlistId || ''))
                  : pathname.endsWith('/remote-playlists/delete') ? await navidromeSync.deleteRemotePlaylists(input)
                    : await navidromeSync.syncNow()
            res.writeHead(200, { 'Content-Type': 'application/json', 'Cache-Control': 'no-store' })
            res.end(JSON.stringify({ success: true, data }))
          } catch (err: any) {
            res.writeHead(400, { 'Content-Type': 'application/json' })
            res.end(JSON.stringify({ success: false, message: err?.message || 'Navidrome 操作失败' }))
          }
        })
        return
      }
...
export const startServer = async (port: number, ip: string) => {
  ...
  playlistSubscription.initialize({
    ...
    apiSyncMode: () => navidromeSync.isApiMode(),
    onPlaylistChanged: () => navidromeSync.scheduleSync(),
  ...
  navidromeSync.initialize({ getPlaylists: () => playlistSubscription.getNavidromeSyncPlans(), reconcile: () => playlistSubscription.reconcilePlaylists() })
